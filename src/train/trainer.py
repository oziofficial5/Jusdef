...
def forward_one_graph(model, g, device):
    ...
    if r2_key in g.edge_types and g[r2_key].edge_index.size(1) > 0:
        edge_attr_dict = {r2_key: {"operator": g[r2_key].operator, "priority": g[r2_key].priority}}
...
def train_one_epoch(model, graphs, seen_mask, label_adj, criterion, optimizer, device, stage):
    model.train()
    total_loss = 0.0
    n_graphs = 0
    for g in graphs:
        ...
        targets = g.y.to(device).unsqueeze(0)
        ...
        loss = criterion(scores, targets, seen_mask.to(device), conc_embs=conc_embs, label_adj=label_adj.to(device) if label_adj is not None else None, active_embs=active_embs, defeated_embs=defeated_embs, stage=stage)
        loss.backward()
...
def evaluate(model, graphs, device):
    ...
    return {"macro_f1": round(float(macro), 4), "micro_f1": round(float(micro), 4), "threshold": round(float(best_thresh), 4)}
...
def train_jusdef(config):
    ...
    model = JusDef(in_dim=768, hidden_dim=config.get("hidden_dim", 512), num_layers=config.get("num_layers", 2), dropout=config.get("dropout", 0.3), temperature=config.get("temperature", 5.0), use_dmp=config.get("use_dmp", True), use_authority=config.get("use_authority", True)).to(device)
    print(f"  Model params: {sum(p.numel() for p in model.parameters()):,}")
    optimizer = torch.optim.AdamW(model.parameters(), lr=config.get("lr", 5e-4), weight_decay=config.get("weight_decay", 0.01))
    criterion = JusDefLoss(lambda1=config.get("lambda1", 0.1), lambda2=config.get("lambda2", 0.1))
    ...
    best_val_f1 = 0.0
    patience_counter = 0
    checkpoint_path = Path(config.get("checkpoint_path", "outputs/checkpoints/best_jusdef.pt"))
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    max_epochs = config.get("epochs", 200)
    print(f"  [DEBUG] starting loop, {max_epochs} epochs")
    for epoch in range(max_epochs):
        stage = get_stage(epoch, config.get("stage1_end", 50), config.get("stage2_end", 100))
        avg_loss = train_one_epoch(model, config["train_graphs"], seen_mask, label_adj, criterion, optimizer, device, stage)
        val_metrics = evaluate(model, config["val_graphs"], device)
        print(f"  Epoch {epoch:3d} [stage {stage}] | loss={avg_loss:.4f} | val_macro={val_metrics['macro_f1']:.4f} | val_micro={val_metrics['micro_f1']:.4f}")
    ...
    results = {"best_val_macro_f1": round(best_val_f1, 4), "test_macro_f1": test_metrics["macro_f1"], "test_micro_f1": test_metrics["micro_f1"], "test_threshold": test_metrics["threshold"]}
