...
def randomize_r2_operators(graphs, seed, n_buckets=4):
    """Replace edge_operator IDs on r2 edges with random bucket IDs."""
    rng = np.random.RandomState(seed)
    for g in graphs:
        if hasattr(g, "edge_operator") and g.edge_operator is not None:
            n = g.edge_operator.numel()
            g.edge_operator = torch.tensor(
                rng.randint(0, n_buckets, size=n), dtype=torch.long
            )
    return graphs
...
def main():
    ...
    parser.add_argument("--capmatch", action="store_true",
                        help="Randomise r2 operator labels (capacity-matched baseline)")
    ...
    if args.capmatch:
        print(f"Applying capacity-matched randomisation (seed {args.seed})")
        train_graphs = randomize_r2_operators(train_graphs, args.seed)
        val_graphs = randomize_r2_operators(val_graphs, args.seed)
        test_graphs = randomize_r2_operators(test_graphs, args.seed)
    ...
    print(f"  DMP={not args.no_dmp}, Authority={not args.no_authority}, Capmatch={args.capmatch}")
    print(f"{'='*50}\n")
